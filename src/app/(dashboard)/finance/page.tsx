...
export default function FinancePage() {
  ...
  const { data: transactions = [], refetch: refetchTransactions } =
    useGetTransactionsQuery();
  const [createTransaction, { isLoading: creatingTxn }] =
    useCreateTransactionMutation();
  const [updateTransaction, { isLoading: updatingTxn }] =
    useUpdateTransactionMutation();
  ...
  const { data: chartAccounts = [], refetch: refetchChart } =
    useGetChartOfAccountsQuery();
  const [createChartAccount, { isLoading: creatingAcct }] =
    useCreateChartAccountMutation();
  const [updateChartAccount, { isLoading: updatingAcct }] =
    useUpdateChartAccountMutation();
  ...
  const totalRevenue =
    invoices
      ...
      .reduce((s: number, i: any) => s + i.amount, 0) || 45000;
  const totalExpenses =
    transactions
      .filter((t: any) => t.type === "Debit")
      .reduce((s: number, t: any) => s + t.amount, 0) || 12500;
  const totalOutstanding =
    invoices
      .filter((i: any) => i.status === "unpaid")
      .reduce((s: number, i: any) => s + i.amount, 0) || 10000;
  ...
  async function confirmDeleteInvoice() {
    try {
      await deleteInvoice(deleteInvoiceTarget.id).unwrap();
  ...
  async function submitTxn(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (txnModal.mode === "create") {
        await createTransaction(payload).unwrap();
        toast.success("Transaction recorded");
      } else {
        await updateTransaction({
          id: txnModal.record.id,
          data: payload,
        }).unwrap();
  ...
  async function confirmDeleteTxn() {
    try {
      await deleteTransaction(deleteTxnTarget.id).unwrap();
  ...
  async function submitAcct(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    try {
      if (acctModal.mode === "create") {
        await createChartAccount(acctForm).unwrap();
        toast.success("Account added");
      } else {
        await updateChartAccount({
          id: acctModal.record.id,
          data: acctForm,
        }).unwrap();
  ...
  async function confirmDeleteAcct() {
    try {
      await deleteChartAccount(deleteAcctTarget.id).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                <tbody>
                  {invoices.slice(0, 5).map((inv: any) => (
                    ...
                    >
                      ...
                      <td className={tdCls}>
                        <InlineStatusBadge status={inv.status} />
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {transactions.slice(0, 5).map((txn: any) => (
                    ...
                    >
                      ...
                      <td className={tdCls}>
                        <InlineStatusBadge status={txn.status} />
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <tbody>
                {invoices.length === 0 && (
                  <tr>
                    ...
                    >
                      No invoices yet. Click &quot;Create Invoice&quot; to add
                      one.
                    </td>
                  </tr>
                )}
                ...
                {transactions.length === 0 && (
                  <tr>
                    <td
                      ...
                      className="py-10 text-center text-sm text-gray-400"
                    >
                      No transactions yet. Click &quot;Record Transaction&quot;
                      to add one.
                    </td>
                  </tr>
                )}
              ...
              <tbody>
                {chartAccounts.length === 0 && (
                  <tr>
                    <td
                      ...
                      className="py-10 text-center text-sm text-gray-400"
                    >
                      No accounts yet. Click &quot;Add Account&quot; to get
                      started.
                    </td>
                  </tr>
                )}
                {chartAccounts.map((acct: any) => (
                  <tr
                    ...
                    className="border-t border-gray-100 hover:bg-gray-50 transition-colors"
                  >
                    <td className={tdCls + " font-mono text-sm font-semibold"}>
                      {acct.code}
                    </td>
                    ...
                    <td className={tdCls}>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          id={`del-coa-${acct.id}`}
                          onClick={() => setDeleteAcctTarget(acct)}
                          className="rounded p-1 text-gray-400 hover:bg-red-50 hover:text-red-600 transition-colors"
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
        ...
        {activeTab === "ledger" && (
          <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div
                className={`rounded-xl border p-5 shadow-sm ${glBalanced ? "border-teal-200 bg-teal-50" : "border-red-200 bg-red-50"}`}
              >
                <p
                  className={`text-sm font-semibold ${glBalanced ? "text-teal-700" : "text-red-700"}`}
                >
                  Ledger Balance Status
                </p>
                <div className="mt-2 flex items-center gap-2">
                  <CheckCircle2
                    className={`h-5 w-5 ${glBalanced ? "text-teal-600" : "text-red-600"}`}
                  />
                  <p
                    className={`text-base font-bold ${glBalanced ? "text-teal-700" : "text-red-700"}`}
                  >
                    {glBalanced
                      ? "Balanced ($0.00 Difference)"
                      : `Unbalanced ($${Math.abs(totalDebits - totalCredits).toFixed(2)} Diff)`}
                  </p>
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
              <div className="px-5 py-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  All recorded ledger postings for the current physical period
                </p>
              </div>
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-100">
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-900">
                    Connected Bank Statement Importer
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Automatically syncs transactions from connected bank
                    accounts
                  </p>
                </div>
              </div>
      ...
      <EntityCreateModal
        ...
        title={
          invoiceModal.mode === "create" ? "Create Invoice" : "Edit Invoice"
        }
        description={
          invoiceModal.mode === "create"
            ? "Add a new invoice for a tenant"
            : "Update the invoice details"
        }
        submitLabel={
          invoiceModal.mode === "create" ? "Create Invoice" : "Save Changes"
        }
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="grid gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setInvoiceForm((p) => ({ ...p, tenant: e.target.value }))
              }
              required
            />
          </div>
          <div className="grid gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setInvoiceForm((p) => ({ ...p, account: e.target.value }))
              }
            />
          </div>
          <div className="grid gap-2">
          ...
          <div className="grid gap-2">
            ...
            <Input
              id="inv-ref"
              placeholder="e.g. ACH-ACME-RENT"
              value={invoiceForm.reference}
              onChange={(e) =>
                setInvoiceForm((p) => ({ ...p, reference: e.target.value }))
              }
            />
          </div>
          <div className="grid gap-2">
      ...
      <EntityCreateModal
        ...
        title={
          txnModal.mode === "create" ? "Record Transaction" : "Edit Transaction"
        }
        description={
          txnModal.mode === "create"
            ? "Record a new financial transaction"
            : "Update transaction details"
        }
        submitLabel={
          txnModal.mode === "create" ? "Record Transaction" : "Save Changes"
        }
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setTxnForm((p) => ({ ...p, account: e.target.value }))
              }
              required
            />
          </div>
          <div className="grid gap-2">
            ...
            <Select
              value={txnForm.type}
              onValueChange={(v) => setTxnForm((p) => ({ ...p, type: v }))}
            >
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setTxnForm((p) => ({ ...p, reference: e.target.value }))
              }
            />
          </div>
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => setTxnForm((p) => ({ ...p, status: v }))}
            >
          ...
          <div className="grid gap-2 sm:col-span-2">
            ...
            <Input
              id="txn-memo"
              placeholder="e.g. Acme Corp rent payment"
              value={txnForm.memo}
              onChange={(e) =>
                setTxnForm((p) => ({ ...p, memo: e.target.value }))
              }
            />
          </div>
        </div>
      ...
      <EntityCreateModal
        ...
        description={
          acctModal.mode === "create"
            ? "Add a new account to the chart of accounts"
            : "Update account details"
        }
        submitLabel={
          acctModal.mode === "create" ? "Add Account" : "Save Changes"
        }
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="grid gap-2">
            ...
            <Select
              value={acctForm.type}
              onValueChange={(v) => setAcctForm((p) => ({ ...p, type: v }))}
            >
          ...
          <div className="grid gap-2">
            ...
            <Select
              value={acctForm.subtype}
              onValueChange={(v) => setAcctForm((p) => ({ ...p, subtype: v }))}
            >
          ...
          <div className="grid gap-2">
            ...
            <Select
              value={acctForm.normalBalance}
              onValueChange={(v) =>
                setAcctForm((p) => ({ ...p, normalBalance: v }))
              }
            >
          ...
          <div className="grid gap-2 sm:col-span-2">
            ...
            <Input
              id="coa-desc"
              placeholder="e.g. Primary operating bank account"
              value={acctForm.description}
              onChange={(e) =>
                setAcctForm((p) => ({ ...p, description: e.target.value }))
              }
            />
          </div>
        </div>
